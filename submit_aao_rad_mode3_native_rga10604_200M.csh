...
# Submit 200M globally unweighted RGA radiative events with mode 3.
#
# This is the simple mode-3 path: every proposal uses
#   (1/Q_l^2, x_l, -t_h, phi_h)
# instead of the legacy (1/Q_l^2, E', cos(theta*), phi_h) coordinates.
# Each job first scans the actual mode-3 proposal, multiplies the observed
# maximum by SCAN_FACTOR, and then uses AAO's stochastic multiplicity rule.
# No separate calibration campaign or fixed envelope is required.
...
# Require an explicit scale so a typo cannot accidentally submit 40,000 jobs.
if ( $#argv != 1 || ( "$argv[1]" != "--smoke" && "$argv[1]" != "--full" ) ) then
    echo "Usage: $0 --smoke|--full"
    echo "  --smoke submits one 100-event job"
...
if ( "$argv[1]" == "--smoke" ) then
    set workflow = "aao_rad_rga10604_mode3_native_smoke"
    set events_per_job = 100
    set jobs = 1
else
    set workflow = "aao_rad_rga10604_mode3_native_200M"
...
while ( $task < $jobs )
    ...
    set input = "$input_dir/mode3_rga10604__g${task_tag}.inp"
