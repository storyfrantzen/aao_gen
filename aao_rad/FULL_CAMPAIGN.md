...
To run a deliberately incomplete majority pilot, provide a duplicate-free
subset of the parent campaign selection.  This is a production filter only;
it does not alter the calibrated envelopes or silently redefine the parent
queue:
...
python3 radiative_full_campaign.py plan-production \
  --campaign "$final_campaign/campaign.json" \
  --calibration "$final_envelopes" \
  --flat-index-file "$selection/production_flat_indices.txt" \
  --output "$pilots" \
  --events-per-stratum 200 \
  --replicas 1 \
  --seed-base 1207000001
```

The filter must be a subset of the parent campaign's immutable flat-index
selection.  The planner rejects duplicates and out-of-parent indices.  The
production campaign snapshots and hashes the original filter plus canonical
`requested_flat_indices.txt`, `ready_flat_indices.txt`, and
`deferred_flat_indices.txt` artifacts.  The deferred list is the complete
parent selection that will not be generated by this campaign; it is never
renormalized into the generated subset.  Without `--flat-index-file`, the
legacy behavior remains unchanged and every ready parent stratum is selected.
